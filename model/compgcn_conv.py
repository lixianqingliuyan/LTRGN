...
class CompGCNConv(MessagePassing):
	def __init__(self, in_channels, out_channels, num_rels, act=lambda x:x, params=None):
		...
		self.A = nn.Linear(2 * in_channels, 1)
	...
	def rel_transform(self, ent_embed, rel_embed):
		...
		elif self.p.opn == 'add': trans_embed = ent_embed + rel_embed
	...
	def message(self, x_i,x_j, edge_type, rel_embed, edge_norm, mode):
		...
		e = torch.cat([x_i, xj_rel], dim=1)
		a = F.leaky_relu(self.A(e))

		out	= torch.mm(xj_rel, weight)

		return out if edge_norm is None else out * edge_norm.view(-1, 1),a
