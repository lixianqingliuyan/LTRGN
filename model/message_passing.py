...
def scatter_(name, src, index, attn,dim_size=None):
	...
	alpha=scatter_softmax(attn,index,dim=0)
	src=alpha*src
...
class MessagePassing(torch.nn.Module):
	...
	def propagate(self, aggr, edge_index, **kwargs):
		...
		kwargs['edge_index'] = edge_index#key=['x', 'edge_type', 'rel_embed', 'edge_norm', 'mode', 'edge_index']

		size = None
		message_args = []
		#self.message_args=[x_i,x_j, edge_type, rel_embed, edge_norm, mode]
		...
		out,attn = self.message(*message_args)
		out = scatter_(aggr, out, edge_index[0], attn, dim_size=size)		# Aggregated neighbors for each vertex
